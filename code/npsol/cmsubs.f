...
*  This version of CMCHK dated  10-Dec-89.       
************************************************************************
      LOGICAL            OK
      INTRINSIC          ABS
      PARAMETER        ( ZERO   =  0.0D+0 , ONE    =  1.0D+0 )
...
C     This version of  CMR1MD  dated 22-Apr-1988.
C***********************************************************************
      INTRINSIC          MIN
...
C***********************************************************************
C     CMRSWP  interchanges the  i-th  and  j-th  (i .lt. j)  columns of
C     an  nrank x n  upper-trapezoidal matrix  R   and restores the
...
C     This version of  CMRSWP  dated  26-Aug-1991.
C***********************************************************************
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      SUBROUTINE CMQMUL( MODE, N, NZ, NFREE, LDZY, UNITQ,
     $                   KX, V, ZY, W )

      IMPLICIT           DOUBLE PRECISION(A-H,O-Z)
      LOGICAL            UNITQ
      INTEGER            KX(N)
      DOUBLE PRECISION   V(N), ZY(LDZY,*), W(N)
...
*        Set  W  =  relevant part of  ZY * V.

         IF (LENV .GT. 0)  THEN
            IF (UNITQ) THEN
               CALL DCOPY ( LENV, V(J1), 1, W(J1), 1 )
            ELSE
               CALL DGEMV ( 'N', NFREE, J2-J1+1, ONE, ZY(1,J1), LDZY,
     $                      V(J1), 1, ONE, W, 1 )
            END IF
         END IF
...
*           Set  V  =  relevant part of  ZY' * W.

            IF (LENV .GT. 0)  THEN
               IF (UNITQ) THEN
                  CALL DCOPY ( LENV, W(J1), 1, V(J1), 1 )
               ELSE
                  CALL DGEMV ( 'T', NFREE, J2-J1+1, ONE, ZY(1,J1), LDZY,
     $                         W, 1, ZERO, V(J1), 1 )
               END IF
            END IF
         END IF
