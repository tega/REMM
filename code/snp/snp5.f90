...
SUBROUTINE SNP()
  ...
  USE S2_COMMON,   ONLY: rho_SM_end, rho_rest_SM_old, SM_nb_par
...
SUBROUTINE SNP_ROBUST()
  ...
  USE S1_COMMON,   ONLY: T1, step, AM_nb_par, beta_AM_start, beta_AM_end, modified_series, &
       pol_dim, derivative_level, inv_score_cov, score_cov, AM_nb_not_fixed_par, &
       write_output_1, write_output_2, log_unit_nb, space_lenght
  USE S2_COMMON,   ONLY: T2, rho_SM_end, rho_rest_SM_old, rho_SM_start, SM_nb_par, &
       change_random_iid
...
101 FORMAT(A,A,TR1,25(ES10.3,TR1))
  ...
  IF (step .NE. 3) THEN
     IF (step .EQ. 1) THEN
        ...
        STOP
     ELSE
        WRITE(*,*) "The robust estimation of the second step only has not been implemeted yes!"
        STOP
     END IF
  END IF
